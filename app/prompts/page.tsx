...
export default function Home() {
  ...
  const variables =
    userPrompt.match(/{{(.*?)}}/g)?.map((v) => v.slice(2, -2)) || [];
  ...
  const handleVariableClick = (variable) => {
    ...
    if (value) {
      setUserPrompt(
        userPrompt.replace(new RegExp(`{{${variable}}}`, "g"), value)
      );
    }
  };
          ...
          <div className="flex flex-wrap justify-center mt-4">
            <div className="result-card w-full md:w-1/3 p-2 bg-white shadow-lg rounded-lg overflow-hidden">
              ...
              <div className="chat-container">
                <div className="chat-message prompt-message bg-gray-100 p-2 rounded">
                  {result.originalPrompt}
                </div>
                <div className="chat-message response-message bg-blue-100 p-2 rounded mt-2">
            ...
            <div className="flex flex-wrap justify-center w-full md:w-2/3">
              {result.improvedPrompts.map((improvedPrompt, index) => (
                <div
                  key={index}
                  className="result-card improved-section w-full md:w-1/2 p-2 bg-white shadow-lg rounded-lg overflow-hidden m-2"
                >
                  <h5 className="font-bold text-lg mb-2">
                    Improved Prompt and Response v{index + 1}
                  </h5>
                  <div className="chat-container">
                    <div className="chat-message prompt-message bg-gray-100 p-2 rounded">
                      {formatResponse(improvedPrompt)}
                    </div>
                    <div className="chat-message response-message bg-blue-100 p-2 rounded mt-2">
                      {formatResponse(result.improvedResponses[index])}
                    </div>
                  </div>
                </div>
              ))}
